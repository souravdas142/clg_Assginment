import java.lang.String;
	...
	class GenSubStr{
		...
		public static void main(String[] str){
			...
			Index[k+1]=str[0].length(); // we've store the length of the string in the last index of Index arry;
			if(k<0)k=0; // handle if k=-1 (throwing IndexOutOfBound exception);

			int length=k+1; // storing the actual length of the Index array(thats mean how much vowel has been found + index at length of string)

			for(i=0;i<length;i++){  //Print the substring in which contain only one vowel that is starting of the substring

				System.out.println(str[0].substring(Index[i],Index[i+1]));
			}
